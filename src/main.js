...
const furnitureColliders=[],playerRadius=.34;
function addCollider(x,z,width,depth){furnitureColliders.push({minX:x-width/2,maxX:x+width/2,minZ:z-depth/2,maxZ:z+depth/2})}
function canOccupy(x,z){return !furnitureColliders.some(c=>x+playerRadius>c.minX&&x-playerRadius<c.maxX&&z+playerRadius>c.minZ&&z-playerRadius<c.maxZ)}
...
const bookPositions=[],secretBookPositions=[],bookGeo=new THREE.BoxGeometry(.48,.76,.09),bookMat=new THREE.MeshStandardMaterial({vertexColors:true,roughness:.8}),bookMesh=new THREE.InstancedMesh(bookGeo,bookMat,96),hiddenBookMatrix=new THREE.Matrix4().makeScale(0,0,0);
const dummy=new THREE.Object3D(),colours=[0x4d1b1d,0x273844,0x55401d,0x37233f,0x3f4d2c,0x63351f];
for(let shelfIndex=0;shelfIndex<4;shelfIndex++)for(let row=0;row<4;row++)for(let i=0;i<6;i++){const side=shelfIndex%2===0?-1:1,x=side*7.86,z=-5.1+shelfIndex*3.1+i*.01,y=1.5+row*1.25;dummy.position.set(x,y,z+(i-2.5)*.72);dummy.rotation.y=Math.PI/2;dummy.updateMatrix();const index=bookPositions.length,colour=colours[index%colours.length];bookMesh.setMatrixAt(index,dummy.matrix);bookMesh.setColorAt(index,new THREE.Color(colour));bookPositions.push({position:dummy.position.clone(),book:books[index%books.length],colour,index,matrix:dummy.matrix.clone()})}
bookMesh.instanceMatrix.needsUpdate=true;bookMesh.instanceColor.needsUpdate=true;hall.add(bookMesh);
const heldBookMesh=new THREE.Mesh(new THREE.BoxGeometry(.44,.72,.09),new THREE.MeshStandardMaterial({color:0x4d1b1d,roughness:.8}));heldBookMesh.position.set(.34,-.34,-.76);heldBookMesh.rotation.set(-.16,-.26,.08);heldBookMesh.visible=false;camera.add(heldBookMesh);let heldBook=null;
...
const secretExit=new THREE.Vector3(0,1.68,-22.65);let secretBuilt=false,secretRoom=null,activeRoom='Grand Hall';
function buildSecretRoom(){if(secretBuilt)return;secretBuilt=true;secretRoom=new THREE.Group();secretRoom.name='Secret Room';roomShell(secretRoom,0,-27,11,10,5.5);const secretLight=new THREE.PointLight(0x7d9b90,12,12,2);secretLight.position.set(0,3,-27);secretRoom.add(secretLight);for(let i=0;i<12;i++){const x=-3.2+(i%6)*1.28,z=-29.2+Math.floor(i/6)*2.1,colour=colours[i%colours.length],b=box(.52,.82,.1,new THREE.MeshStandardMaterial({color:colour}),x,1.1,z,secretRoom),book=books[i%books.length];b.userData={kind:'book',book};secretBookPositions.push({position:new THREE.Vector3(x,1.1,z),book,colour,mesh:b})}box(3.2,.08,1.8,MAT.wood,0,1.25,-26,secretRoom);addCollider(0,-26,3.35,1.95);readingSpots.push(new THREE.Vector3(0,1.68,-24.7));const exit=box(2.4,4,.08,MAT.darkWood,0,2,-22.38,secretRoom);exit.userData={kind:'exit'};scene.add(secretRoom)}
function openBook(book){const key='library-after-dark-2-read-'+book[0],opened=Number(localStorage.getItem(key)||0)+1;localStorage.setItem(key,String(opened));readerTitle.textContent=book[0];readerAuthor.textContent=book[1];readerCopy.textContent=book[2]+'\n\nThis copy has been opened '+opened+' time'+(opened===1?'':'s')+' on this device.';reader.hidden=false;document.exitPointerLock?.()}
function nearestBook(items){return items.reduce((best,item)=>item.position.distanceToSquared(player)<best.distance?{item,distance:item.position.distanceToSquared(player)}:best,{item:null,distance:6.5})}
function isNearReadingSpot(){return readingSpots.some(spot=>spot.distanceToSquared(player)<7)}
function takeBook(item){heldBook=item;heldBookMesh.material.color.setHex(item.colour);heldBookMesh.visible=true;if(item.mesh)item.mesh.visible=false;else{bookMesh.setMatrixAt(item.index,hiddenBookMatrix);bookMesh.instanceMatrix.needsUpdate=true}status.textContent='Carrying '+item.book[0]}
function returnBook(){if(!heldBook)return;if(heldBook.mesh)heldBook.mesh.visible=true;else{bookMesh.setMatrixAt(heldBook.index,heldBook.matrix);bookMesh.instanceMatrix.needsUpdate=true}heldBook=null;heldBookMesh.visible=false;status.textContent=activeRoom}
function interact(){if(!veil.classList.contains('leaving')||!reader.hidden)return;if(heldBook){if(activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10){secretRoom.visible=false;player.set(0,1.68,9);activeRoom='Grand Hall';status.textContent='Carrying '+heldBook.book[0];return}if(isNearReadingSpot()){openBook(heldBook.book);return}if(activeRoom==='Grand Hall'&&player.distanceToSquared(returnSpot)<9){returnBook();return}return}const nearest=nearestBook(activeRoom==='Grand Hall'?bookPositions:secretBookPositions);if(nearest.item){takeBook(nearest.item);return}if(activeRoom==='Grand Hall'&&player.distanceToSquared(new THREE.Vector3(0,1.68,-10.5))<15){buildSecretRoom();secretRoom.visible=true;player.set(0,1.68,-23);activeRoom='Secret Room';status.textContent=activeRoom;return}if(activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10){secretRoom.visible=false;player.set(0,1.68,9);activeRoom='Grand Hall';status.textContent=activeRoom}}

let monitorOn=false,last=performance.now(),frames=0,started=false;
function setPrompt(){if(!started||!reader.hidden){prompt.style.opacity=0;return}if(heldBook){const atReadingSpot=isNearReadingSpot(),atReturnSpot=activeRoom==='Grand Hall'&&player.distanceToSquared(returnSpot)<9,atExit=activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10;prompt.textContent=atExit?'E  —  take '+heldBook.book[0]+' back to the hall':atReadingSpot?'E  —  sit and read '+heldBook.book[0]:atReturnSpot?'E  —  return '+heldBook.book[0]:'Carry '+heldBook.book[0]+' to a chair or desk';prompt.style.opacity=1;return}const nearDoor=activeRoom==='Grand Hall'&&player.distanceToSquared(new THREE.Vector3(0,1.68,-10.5))<15,nearExit=activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10,nearBook=(activeRoom==='Grand Hall'?bookPositions:secretBookPositions).some(item=>item.position.distanceToSquared(player)<6.5);prompt.textContent=nearDoor?'E  —  enter the hidden room':nearExit?'E  —  return to the Grand Hall':nearBook?'E  —  take a book':' ';prompt.style.opacity=(nearDoor||nearExit||nearBook)?1:0}
function animate(now){requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.04);if(started&&reader.hidden){const speed=keys.ShiftLeft?4.5:2.6;const forward=new THREE.Vector3(-Math.sin(look.yaw),0,-Math.cos(look.yaw)),right=new THREE.Vector3(-forward.z,0,forward.x),move=new THREE.Vector3();if(keys.KeyW)move.add(forward);if(keys.KeyS)move.sub(forward);if(keys.KeyA)move.sub(right);if(keys.KeyD)move.add(right);if(move.lengthSq()){move.normalize().multiplyScalar(speed*dt);const limit=activeRoom==='Grand Hall'?8.4:5,nextX=THREE.MathUtils.clamp(player.x+move.x,-limit,limit),nextZ=THREE.MathUtils.clamp(player.z+move.z,activeRoom==='Grand Hall'?-11:-31.5,activeRoom==='Grand Hall'?11:-22.5);if(canOccupy(nextX,player.z))player.x=nextX;if(canOccupy(player.x,nextZ))player.z=nextZ}camera.position.copy(player);camera.rotation.set(look.pitch,look.yaw,0,'YXZ');setPrompt()}renderer.render(scene,camera);frames++;if(now-last>1000){if(monitorOn){const r=renderer.info.render;monitor.textContent=`${Math.round(frames*1000/(now-last))} FPS\n${r.calls} draw calls\n${Math.round(r.triangles/1000)}k triangles\nroom: ${activeRoom}`}last=now;frames=0}}
animate();
