...
const bookPositions=[],secretBookPositions=[],bookGeo=new THREE.BoxGeometry(.48,.76,.09),bookMat=new THREE.MeshStandardMaterial({vertexColors:true,roughness:.8}),bookMesh=new THREE.InstancedMesh(bookGeo,bookMat,96);
...
function buildSecretRoom(){if(secretBuilt)return;secretBuilt=true;secretRoom=new THREE.Group();secretRoom.name='Secret Room';roomShell(secretRoom,0,-27,11,10,5.5);const secretLight=new THREE.PointLight(0x7d9b90,12,12,2);secretLight.position.set(0,3,-27);secretRoom.add(secretLight);for(let i=0;i<12;i++){const x=-3.2+(i%6)*1.28,z=-29.2+Math.floor(i/6)*2.1,b=box(.52,.82,.1,new THREE.MeshStandardMaterial({color:colours[i%colours.length]}),x,1.1,z,secretRoom),book=books[i%books.length];b.userData={kind:'book',book};secretBookPositions.push({position:new THREE.Vector3(x,1.1,z),book})}box(3.2,.08,1.8,MAT.wood,0,1.25,-26,secretRoom);const exit=box(2.4,4,.08,MAT.darkWood,0,2,-22.38,secretRoom);exit.userData={kind:'exit'};scene.add(secretRoom)}
...
function interact(){if(!veil.classList.contains('leaving')||!reader.hidden)return;const nearest=nearestBook(activeRoom==='Grand Hall'?bookPositions:secretBookPositions);if(nearest.item){openBook(nearest.item.book);return}if(activeRoom==='Grand Hall'&&player.distanceToSquared(new THREE.Vector3(0,1.68,-10.5))<15){buildSecretRoom();secretRoom.visible=true;player.set(0,1.68,-23);activeRoom='Secret Room';status.textContent=activeRoom;return}if(activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10){secretRoom.visible=false;player.set(0,1.68,9);activeRoom='Grand Hall';status.textContent=activeRoom}}

let monitorOn=false,last=performance.now(),frames=0,started=false;
function setPrompt(){if(!started||!reader.hidden){prompt.style.opacity=0;return}const nearDoor=activeRoom==='Grand Hall'&&player.distanceToSquared(new THREE.Vector3(0,1.68,-10.5))<15,nearExit=activeRoom==='Secret Room'&&player.distanceToSquared(secretExit)<10,nearBook=(activeRoom==='Grand Hall'?bookPositions:secretBookPositions).some(item=>item.position.distanceToSquared(player)<6.5);prompt.textContent=nearDoor?'E  —  enter the hidden room':nearExit?'E  —  return to the Grand Hall':nearBook?'E  —  take a book':' ';prompt.style.opacity=(nearDoor||nearExit||nearBook)?1:0}
